...
export async function POST(request: NextRequest) {
  ...
  if (proposal.status === "discarded") {
    return NextResponse.json(
      { success: false, error: "This proposal was discarded." },
      { status: 409 }
    );
  }

  // In the wp-admin editor, clicking Apply IS the approval. Auto-approve a
  // draft so the one-click flow works, keeping every guarantee after it.
