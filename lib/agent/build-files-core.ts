...
9. assets/css/components.css: exactly the COMPONENT CSS when provided — the rules the designed inner pages add on top of the homepage: forms, .entry body typography, tables, blockquotes, and whatever shapes those pages introduced. Do not re-derive or trim it; the templates below depend on these classes existing.
